...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        if self.training:
            ...
            if self.decode_in_inference:
                return self.decode_outputs(outputs, device=xin[0].device)
    ...
    def decode_outputs(self, outputs, device):
        grids = []
        strides = []
        ...
        grids = torch.cat(grids, dim=1).to(device)  # .type(dtype)
        strides = torch.cat(strides, dim=1).to(device)  # .type(dtype)
